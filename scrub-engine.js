...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const c = copies[i];
      c.style.opacity = cop;
      c.style.transform = (reduce || compact) ? 'none' : `translateY(calc(-50% + ${(0.5 - pr) * 4}vh))`;
  ...
  if (smallMQ.addEventListener) smallMQ.addEventListener('change', layout);
  layout();
  requestAnimationFrame(raf);
...
function injectCSS() {
  ...
  html,body{margin:0;background:var(--sw-bg,#F5EDE0);overflow-x:hidden;width:100%;max-width:100%;}
  .sw-sky{position:fixed;inset:0;z-index:0;overflow:hidden;pointer-events:none;background:var(--sw-bg);width:100vw;height:100%;height:100dvh;}
  ...
  .sw-topbar{position:fixed;top:0;left:0;right:0;z-index:50;display:flex;align-items:center;justify-content:space-between;gap:12px;
    padding:calc(env(safe-area-inset-top,0px) + clamp(10px,2vw,22px)) max(12px, env(safe-area-inset-right,0px) + clamp(12px,4vw,64px)) clamp(10px,2vw,22px) max(12px, env(safe-area-inset-left,0px) + clamp(12px,4vw,64px));}
  .sw-brand{display:flex;align-items:center;gap:10px;text-decoration:none;color:var(--sw-ink);flex:0 0 auto;min-width:0;}
  .sw-brand__mark{width:24px;height:28px;flex:0 0 auto;border-radius:7px 7px 10px 10px;background:linear-gradient(160deg,var(--sw-accent),color-mix(in srgb,var(--sw-accent) 60%,#000));box-shadow:0 6px 14px color-mix(in srgb,var(--sw-accent) 40%,transparent);}
  .sw-brand__name{font-family:var(--sw-font-display);font-weight:700;font-size:clamp(.78rem,2.2vw,1.1rem);letter-spacing:.18em;}
  .sw-nav{display:flex;flex-wrap:nowrap;gap:2px;padding:5px;background:color-mix(in srgb,#fff 55%,transparent);backdrop-filter:blur(10px);border:1px solid color-mix(in srgb,var(--sw-accent) 16%,transparent);border-radius:999px;max-width:min(70vw,720px);overflow-x:auto;scrollbar-width:none;-webkit-overflow-scrolling:touch;}
  .sw-nav::-webkit-scrollbar{display:none;}
  .sw-nav__item{font:inherit;font-size:clamp(.72rem,1.15vw,.82rem);color:var(--sw-ink-soft);border:0;background:transparent;cursor:pointer;padding:7px 12px;border-radius:999px;transition:color .25s,background .25s;white-space:nowrap;flex:0 0 auto;}
  ...
  .sw-copy{position:absolute;left:max(16px, env(safe-area-inset-left,0px) + clamp(14px,4vw,64px));top:50%;transform:translateY(-50%);
    width:min(42vw,460px);max-width:calc(100vw - 96px);opacity:0;will-change:opacity,transform;}
  .sw-copy__num{font-family:ui-monospace,Menlo,monospace;font-size:.74rem;letter-spacing:.12em;color:var(--sw-ink-soft);}
  .sw-copy__eyebrow{display:block;margin-top:clamp(10px,2vh,18px);font-family:var(--sw-font-display);font-weight:700;font-size:.8rem;letter-spacing:.16em;text-transform:uppercase;color:var(--sw-accent);}
  .sw-copy__title{font-family:var(--sw-font-display);font-weight:700;color:var(--sw-ink);font-size:clamp(1.7rem,4.2vw,3.5rem);line-height:1.03;margin:12px 0 0;letter-spacing:-.01em;text-shadow:0 2px 20px color-mix(in srgb,var(--sw-bg) 70%,transparent);}
  .sw-copy__body{margin-top:clamp(10px,2vh,18px);font-size:clamp(.95rem,1.25vw,1.14rem);line-height:1.55;color:color-mix(in srgb,var(--sw-ink) 78%,var(--sw-ink-soft));max-width:40ch;text-shadow:0 1px 12px color-mix(in srgb,var(--sw-bg) 90%,transparent);}
  .sw-copy__tags{list-style:none;display:flex;flex-wrap:wrap;gap:8px;margin:clamp(12px,3vh,24px) 0 0;padding:0;}
  ...
  .sw-copy__cta{display:flex;flex-wrap:wrap;gap:12px;margin-top:clamp(16px,3vh,28px);pointer-events:auto;}
  ...
  .sw-route{position:fixed;right:max(6px, env(safe-area-inset-right,0px) + clamp(8px,2vw,30px));top:50%;z-index:40;transform:translateY(-50%);display:flex;flex-direction:column;gap:22px;padding:18px 10px;}
  ...
  .sw-hint{position:fixed;left:50%;bottom:calc(18px + env(safe-area-inset-bottom,0px));z-index:30;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;gap:10px;font-size:.76rem;letter-spacing:.14em;text-transform:uppercase;color:var(--sw-ink-soft);transition:opacity .3s;}
  ...
  @media (max-width:1180px){
    .sw-nav{display:none;}
    .sw-copy{width:min(48vw,420px);}
  }
  @media (max-width:860px){
    .sw-topcta{padding:8px 14px;}
    .sw-copylayer::before{width:100%;height:58%;top:auto;bottom:0;background:linear-gradient(0deg,var(--sw-bg) 10%,color-mix(in srgb,var(--sw-bg) 72%,transparent) 48%,transparent 100%);}
    .sw-copy{left:max(16px, env(safe-area-inset-left,0px) + 16px);right:max(40px, env(safe-area-inset-right,0px) + 28px);
      top:auto;bottom:calc(clamp(52px,11dvh,108px) + env(safe-area-inset-bottom,0px));transform:none;width:auto;max-width:none;}
    .sw-copy__title{font-size:clamp(1.7rem,7vw,2.6rem);}
    .sw-copy__body{max-width:none;font-size:clamp(.92rem,3.5vw,1.08rem);}
    .sw-scene__video,.sw-scene__still{object-position:center 46%;}
    .sw-hint{bottom:calc(14px + env(safe-area-inset-bottom,0px));}
    .sw-route{gap:14px;right:max(4px, env(safe-area-inset-right,0px));padding:10px 4px;}
    .sw-route__label{display:none;}
  }
  @media (max-width:520px){
    .sw-brand__name{letter-spacing:.14em;}
    .sw-copy__tags li:nth-child(n+3){display:none;}
  ...
  @media (max-height:540px) and (orientation:landscape){
    .sw-nav,.sw-hint,.sw-copy__tags,.sw-copy__body{display:none;}
    .sw-topbar{padding:calc(env(safe-area-inset-top,0px) + 6px) 16px 6px;}
    .sw-copylayer::before{width:min(46vw,420px);height:100%;background:linear-gradient(90deg,var(--sw-bg) 0%,color-mix(in srgb,var(--sw-bg) 55%,transparent) 55%,transparent 100%);}
    .sw-copy{left:max(14px, env(safe-area-inset-left,0px) + 14px);right:auto;top:50%;bottom:auto;transform:translateY(-50%);width:min(46vw,380px);}
    .sw-copy__title{font-size:clamp(1.3rem,4.2vw,2rem);margin-top:6px;}
    .sw-copy__eyebrow{margin-top:6px;}
    .sw-copy__cta{margin-top:10px;}
    .sw-route{gap:10px;}
  }
